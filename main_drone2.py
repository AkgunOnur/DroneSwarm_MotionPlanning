...
parser.add_argument('--mode',  default="Train",
                    help="Train or Test or Report mode")
...
elif args.mode == "Train" or args.mode == "Test" or args.mode == "Multi-Test" or args.mode == "Report":
    env = QuadrotorFormation(n_agents=n_agents, N_frame=N_frame,
...
if args.mode == "Train":
    ...
    print ("Multi-Test mode is executed! \n")
    file_list = glob.glob(".\model\*.pt")
    for i, file in enumerate(file_list):
        print("{0}/{1} file {2} is loaded! \n".format(i+1, len(file_list), file))
        load(file)
        pkl_name = os.path.split(os.path.splitext(file)[0])[1] + '.pkl'
        tester.test_batch(file_name=pkl_name)
