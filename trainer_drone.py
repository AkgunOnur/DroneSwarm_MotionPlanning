...
class Trainer(object):
    def __init__(self, args, policy_net, env, is_centralized=False):
        ...
        self.state_arr = np.zeros((self.N_iteration, self.args.nagents, (
            self.args.nagents + 1) * 5 + 2, self.env.out_shape, self.env.out_shape))
        self.next_state_arr = np.zeros((self.N_iteration, self.args.nagents, (
            self.args.nagents + 1) * 5 + 2, self.env.out_shape, self.env.out_shape))
        self.action_arr = np.zeros((self.N_iteration, self.args.nagents))
        self.action_out_arr = np.zeros(
            (self.N_iteration, self.args.nagents, self.env.n_action))
        ...
        self.episode_mini_mask_arr = np.zeros(
            (self.N_iteration, self.args.nagents))
    ...
    def get_episode(self, epoch):
        ...
        for t in range(self.N_iteration):
            ...
            state = state.view(self.args.nagents, state.size(-3),
            ...
            total_obs, reward, done, info = self.env.step(
                action, t, self.is_centralized)
            next_state, battery_status = total_obs
            print("T-Episode/Iteration: {0}/{1}, Actions: {2}, Rewards: {3}".format(
                epoch + 1, t + 1, action[0], reward))
            ...
            self.action_out_arr[t, :, :] = np.copy(
                action_out[0].detach().numpy())
        ...
        batch = self.state_arr[0:final_step], self.action_arr[0:final_step], self.action_out_arr[0:final_step], self.value_arr[0:final_step], self.episode_mask_arr[0:
                                                                                                                                                                    final_step], self.episode_mini_mask_arr[0:final_step], self.next_state_arr[0:final_step], self.reward_arr[0:final_step], self.misc_arr[0:final_step]
    ...
    def run_batch(self, epoch):
        ...
        state_total_batch = np.array([]).reshape(
            (0, self.args.nagents, (self.args.nagents + 1) * 5 + 2, self.env.out_shape, self.env.out_shape))
        next_state_total_batch = np.array([]).reshape(
            (0, self.args.nagents, (self.args.nagents + 1) * 5 + 2, self.env.out_shape, self.env.out_shape))
        action_total_batch = np.array([]).reshape((0, self.args.nagents))
        action_out_total_batch = np.array([]).reshape(
            (0, self.args.nagents, self.env.n_action))
        value_total_batch = np.array([]).reshape((0, self.args.nagents))
        episode_mask_total_batch = np.array([]).reshape((0, self.args.nagents))
        episode_mini_mask_total_batch = np.array(
            []).reshape((0, self.args.nagents))
        ...
        while state_total_batch.shape[0] < self.args.batch_size:
            ...
            next_state_total_batch = np.vstack(
                [next_state_total_batch, next_state_batch])
            action_total_batch = np.vstack([action_total_batch, action_batch])
            action_out_total_batch = np.vstack(
                [action_out_total_batch, action_out_batch])
            value_total_batch = np.vstack([value_total_batch, value_batch])
            episode_mask_total_batch = np.vstack(
                [episode_mask_total_batch, episode_mask_batch])
            episode_mini_mask_total_batch = np.vstack(
                [episode_mini_mask_total_batch, episode_mini_mask_batch])
...
class Tester(Trainer):
    ...
    def test_batch(self, pkl_name, save=True):
        batch = []
        N_epoch = 10
        total_pos_list = []
        for epoch in range(N_epoch):
            ...
            if save:
                ...
                with open('./agents_position/' + pkl_name, 'wb') as f:
