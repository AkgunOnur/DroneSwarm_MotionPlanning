...
class DQNBase(BaseNetwork):
    ...
    def forward(self, agent_obs):
        agent_obs = agent_obs.view(-1, agent_obs.size()[-3], agent_obs.size()[-2], agent_obs.size()[-1])
        return self.net(agent_obs)
...
class CateoricalPolicy(BaseNetwork):
    ...
    def act(self, agent_obs, device):
        agent_obs = torch.FloatTensor(agent_obs).to(device)
        if not self.shared:
            conv_fcn = self.conv(agent_obs)

        action_logits = self.head(conv_fcn)
        greedy_actions = torch.argmax(
            action_logits, dim=1, keepdim=True)
        return greedy_actions

    def sample(self, agent_obs, device):
        agent_obs = torch.FloatTensor(agent_obs).to(device)
        if not self.shared:
            conv_fcn = self.conv(agent_obs)

        action_probs = F.softmax(self.head(conv_fcn), dim=1)
