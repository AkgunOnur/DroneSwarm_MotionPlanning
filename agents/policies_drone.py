...
from agents.utils_drone import batch_to_seq, init_layer, one_hot, run_rnn
...
class Policy(nn.Module):
    ...
    def _init_actor_head(self, n_h, n_a=None):
        ...
        self.actor_head = nn.Linear(self.num_inputs, n_a)
...
class LstmPolicy(Policy):
    ...
    def backward(self, obs, nactions, acts, dones, Rs, Advs,
                 e_coef, v_coef, global_step=None):
...
class FPPolicy(LstmPolicy):
    ...
    def _init_net(self):
        ...
        self.fc_x_layer = nn.Linear(self.num_inputs, self.n_fc)
...
class NCMultiAgentPolicy(Policy):
    ...
    def _init_comm_layer(self, n_n, n_ns, n_na):
        ...
        fc_x_layer = nn.Linear(self.num_inputs, self.n_fc)
            ...
            for i in range(self.n_agents):
                ...
                if n_n:
                    ...
                    if self.identical:
                        ...
                        x_i = x[i].narrow(0, 0, self.n_s_ls[i]).unsqueeze(0)
                    # print ("x_i: ",x_i.size())
                    encoded_x = self.net(x_i)
                    # print ("encoded_x: ",encoded_x.size())
                    # print ("self.fc_x_layers[i]: ", self.fc_x_layers[i])
                    s_i = F.relu(self.fc_x_layers[i](encoded_x))
