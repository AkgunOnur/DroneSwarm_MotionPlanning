...
<<<<<<< HEAD
    ...
    def desiredState(self, t, Ts, state):
...
=======
        ...
        if (self.xyzType >= 3 and self.xyzType <= 6):
            ...
            # Calculate coefficients
            self.coeff_x = minSomethingTraj(
                self.wps[:, 0], self.T_segment, self.deriv_order)
            self.coeff_y = minSomethingTraj(
                self.wps[:, 1], self.T_segment, self.deriv_order)
            self.coeff_z = minSomethingTraj(
            ...
            # Calculate coefficients
            self.coeff_x = minSomethingTraj_stop(
                self.wps[:, 0], self.T_segment, self.deriv_order)
            self.coeff_y = minSomethingTraj_stop(
                self.wps[:, 1], self.T_segment, self.deriv_order)
            self.coeff_z = minSomethingTraj_stop(
                self.wps[:, 2], self.T_segment, self.deriv_order)
        ...
        self.desPos = np.zeros(3)    # Desired position (x, y, z)
        self.desVel = np.zeros(3)    # Desired velocity (xdot, ydot, zdot)
        # Desired acceleration (xdotdot, ydotdot, zdotdot)
        self.desAcc = np.zeros(3)
        # Desired thrust in N-E-D directions (or E-N-U, if selected)
        self.desThr = np.zeros(3)
        # Desired orientation in the world frame (phi, theta, psi)
        self.desEul = np.zeros(3)
        # Desired angular velocity in the body frame (p, q, r)
    ...
    def desiredState(self, t, Ts, state):
        ...
        def pos_waypoint_timed():
            ...
            if not (len(self.t_wps) == self.wps.shape[0]):
                raise Exception(
                    "Time array and waypoint array not the same size.")
            elif (np.diff(self.t_wps) <= 0).any():
                raise Exception("Time array isn't properly ordered.")
        ...
        def pos_waypoint_interp():
            ...
            if (t == 0):
                ...
                scale = (t - self.t_wps[self.t_idx]) / \
                    self.T_segment[self.t_idx]
                self.desPos = (
                    1 - scale) * self.wps[self.t_idx, :] + scale * self.wps[self.t_idx + 1, :]
...
=======
            ...
            # Stay hover at the last waypoint position
                ...
                # Set desired position, velocity and acceleration
                t0 = get_poly_cc(nb_coeff, 0, scale)
                self.desPos = np.array([self.coeff_x[start:end].dot(
                    t0), self.coeff_y[start:end].dot(t0), self.coeff_z[start:end].dot(t0)])

                t1 = get_poly_cc(nb_coeff, 1, scale)
                self.desVel = np.array([self.coeff_x[start:end].dot(
                    t1), self.coeff_y[start:end].dot(t1), self.coeff_z[start:end].dot(t1)])

                t2 = get_poly_cc(nb_coeff, 2, scale)
                self.desAcc = np.array([self.coeff_x[start:end].dot(
                    t2), self.coeff_y[start:end].dot(t2), self.coeff_z[start:end].dot(t2)])
...
<<<<<<< HEAD
                distance_to_next_wp = ((self.wps[self.t_idx,0]-state[0])**2 + (self.wps[self.t_idx,1]-state[1])**2 + (self.wps[self.t_idx,2]-state[2])**2)**(0.5)
...
=======
                distance_to_next_wp = ((self.wps[self.t_idx, 0] - state[0])**2 + (
                    self.wps[self.t_idx, 1] - state[1])**2 + (self.wps[self.t_idx, 2] - state[2])**2)**(0.5)
                if (distance_to_next_wp < dist_consider_arrived):
                    self.t_idx += 1
...
=======
                scale = (t - self.t_wps[self.t_idx]) / \
                    self.T_segment[self.t_idx]
                self.desEul[2] = (1 - scale) * self.y_wps[self.t_idx] + \
                    scale * self.y_wps[self.t_idx + 1]
...
=======
                raise Exception(
                    "Function yaw_follow isn't compatible with selected xyzType trajectory")
>>>>>>> 65cda8f9482f7302bd187f13dee54c08ca1c2739
...
<<<<<<< HEAD
                self.sDes = np.hstack((self.desPos, self.desVel, self.desAcc, self.desEul)).astype(float)
        
=======
                self.sDes = np.hstack(
                    (self.desPos, self.desVel, self.desAcc, self.desEul)).astype(float)

>>>>>>> 65cda8f9482f7302bd187f13dee54c08ca1c2739
        return self.desPos, self.desVel, self.desAcc, self.desEul
...
=======
    ...
    # Constraint 2 - Ending position for every segment
    for i in range(n):
        A[i + n][nb_coeff * i:nb_coeff *
                 (i + 1)] = get_poly_cc(nb_coeff, 0, times[i])
    ...
    for k in range(1, order):
        A[2 * n + (order - 1) + k -
          1][-nb_coeff:] = get_poly_cc(nb_coeff, k, times[i])

    # Constraint 5 - All derivatives are continuous at each waypint transition
    for i in range(n - 1):
        for k in range(1, nb_coeff - 1):
            A[2 * n + 2 * (order - 1) + i * 2 * (order - 1) + k - 1][i * nb_coeff: (i * nb_coeff + nb_coeff * 2)
...
=======
            A[2 * n + k - 1 + i * (order - 1)][nb_coeff *
                                               i:nb_coeff * (i + 1)] = get_poly_cc(nb_coeff, k, 0)
>>>>>>> 65cda8f9482f7302bd187f13dee54c08ca1c2739
...
=======
            A[2 * n + (order - 1) * n + k - 1 + i * (order - 1)][nb_coeff *
                                                                 i:nb_coeff * (i + 1)] = get_poly_cc(nb_coeff, k, times[i])
...
=======
        A[i + 2 * n][nb_coeff * i:nb_coeff *
                     (i + 1)] = get_poly_cc(nb_coeff, 1, 0)
>>>>>>> 65cda8f9482f7302bd187f13dee54c08ca1c2739
...
=======
        A[i + 3 * n][nb_coeff * i:nb_coeff *
                     (i + 1)] = get_poly_cc(nb_coeff, 1, times[i])
>>>>>>> 65cda8f9482f7302bd187f13dee54c08ca1c2739
...
=======
        A[4 * n + k - 2 + (order - 2)
          ][-nb_coeff:] = get_poly_cc(nb_coeff, k, times[i])

    # Constraint 7 - All derivatives above velocity are continuous at each waypint transition
    for i in range(n - 1):
        for k in range(2, nb_coeff - 2):
            # print(4*n+2*(order-2)+k-2+i*(nb_coeff-4))
            A[4 * n + 2 * (order - 2) + k - 2 + i * (nb_coeff - 4)][i * nb_coeff: (i * nb_coeff + nb_coeff * 2)
                                                                    ] = np.concatenate((get_poly_cc(nb_coeff, k, times[i]), -get_poly_cc(nb_coeff, k, 0)))
>>>>>>> 65cda8f9482f7302bd187f13dee54c08ca1c2739
...
=======
    ...
    sDes = np.hstack((desPos, desVel, desAcc, desThr, desEul,
                      desPQR, desYawRate)).astype(float)
>>>>>>> 65cda8f9482f7302bd187f13dee54c08ca1c2739
